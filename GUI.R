...
Text_X_Max <- max(Data$ID)
Rect_X_With <- length(Data$ID)
...
server <- function(input,output){
  ...
  dat <- reactive({
    print(input$num)
    Data[min(input$num):max(input$num),]
  })
  ...
  output$Alture <- renderPlot({
    ggplot(dat(), aes(x=ID,y=ALT_B)) +
      geom_ribbon(aes(xmin=Graph_X_Min, xmax=Graph_X_Max,
                  ...
                  fill="#606437ff", alpha=0.2) +
      geom_line(color="#a8b063ff", alpha=0.6) +
      coord_cartesian(xlim = c(Graph_X_Min,Graph_X_Max),
                      ylim = c(Graph_Y_Min,Graph_Y_Max),
                      expand = FALSE) +
      dark_theme_classic() +
      scale_x_continuous(breaks = seq(0, Graph_X_Max, by = 50)) +
      scale_y_continuous(breaks = seq(Graph_Y_Min, Graph_Y_Max, by=1)) +
      theme(plot.margin = unit(c(0.2,0.5,0.2,0.2), "cm"),
            plot.background = element_rect(fill = "#282d32ff"),
            panel.background = element_rect(fill = "#282d32ff")) +
      ...
      #### <- here is the problem witrh x = ID[dat()$ALT_B == max(dat()$ALT_B)]
      geom_segment(aes(x = dat()$ID[max(dat()$ALT_B)], y = max(dat()$ALT_B), xend = Rect_X_With, yend = max(dat()$ALT_B)), linetype = 2, colour = 'grey') + 
      #geom_segment(aes(x= ID[dat()$ALT_B == max(dat()$ALT_B)], y = max(dat()$ALT_B), xend = Rect_X_With, yend = max(dat()$ALT_B)), linetype = 2, colour = 'grey') + 
      #geom_text(aes(x = Text_X_Max, y = ALT_B[ALT_B == max(ALT_B)] , label = max(ALT_B)), hjust = -0.1) +
